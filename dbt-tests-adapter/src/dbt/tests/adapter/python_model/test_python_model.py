...
def model(dbt, session):
    ...
    # Use meta_get to access meta values
    owner = dbt.meta_get('owner')
    priority = dbt.meta_get('priority', 'low')
    version = dbt.meta_get('version')
    missing = dbt.meta_get('nonexistent', 'default-value')
...
def model(dbt, session):
    ...
    owner = dbt.meta_get('owner')
    environment = dbt.meta_get('environment', 'dev')
...
class BasePythonMetaGetTests:
    """Test dbt.meta_get() functionality in Python models"""
    ...
    def test_meta_get_basic(self, project):
        """Test that dbt.meta_get() works with meta defined in Python model"""
